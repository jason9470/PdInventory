...
| `EmployeesController` | **共用** | 人員（員編＋姓名＋部門＋組別＋科別） | `/Employees` |
...
- **維護資料表裡有一批「代號待補」的資料**：既有資料中出現、但不在甲方 0903 清單裡的
  ...
  要請甲方一起清。人員表的 員編 位數也不一致（有 `7331` 也有 `0002876`），
  是來源檔原樣沒有補零；將來若要與 `AppUsers` 對接，`LdapEmployeeDirectory.NormalizeEmpNo`
  可以補到七碼。
