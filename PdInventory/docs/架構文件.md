...
| `SectionsController` | **共用** | **科別**（科／組／部室，以及各科負責的系統）——**權限的核心**，見 8.2 | `/Sections` |
...
| `PermissionsController` | 權限設定：調整使用者角色；能修改哪些系統唯讀顯示（限管理者） |
...
刪除限管理者的理由：SW 與 DA 清單的刪除是**整筆資產連同 DA 與盤點表一起刪**（見 3.4），
修改權限依科別開放給所有人之後，連帶開放刪除的風險太大。風險自評的「刪除」實際上是清空
風險欄位，同樣只限管理者。
...
權限設定畫面的清單、科別畫面的負責系統全部走它，**畫面上看到的範圍就是實際生效的範圍**。
...
| 單筆層 | `IAssetAccess.CanModifyAsync(systemCode)`／`CanDelete` | `Edit`／`Delete` 動作內 |
...
| 權限設定 `AppUsers` | 能不能登入、什麼角色（能改哪些系統看人員表的科別） |
...
2. **管理者先在人員表建檔**——存檔時一併建立帳號（同樣預設一般使用者），
...
**人員表的每個人在權限設定都看得到。** 兩條新增路徑本來就會同時建立兩邊，
...
- 再擋「刪掉就沒人能管了」：不能刪自己，也不能刪掉最後一位管理者。
  權限設定畫面本來就有這兩道保護，但**從人員表刪除是另一條路、繞過了它們**——
...
| `Employee`／`AppUser` | 人員／權限設定（兩邊一起，見 8.5） |
...
- **來源試算表的「修改者／修改時間」由系統寫入**（0910）：`SwModifiedBy`／`SwModifiedTime`／
  `DaModifiedTime` 三欄在 `AppDbContext.StampSourceSheetModifiedFields()` 存檔時填入目前登入者
  與現在時間，畫面上是唯讀的。格式沿用來源資料的寫法（`2026/8/28 08:16`、修改者只寫姓名），
  因為這三欄要跟著匯出檔回到甲方的 Excel——這也是它們不能直接併進 `CreatedBy`／`UpdatedBy`
  那組系統軌跡的原因。沒有登入者時（種子匯入）不寫，否則會把來源試算表原本的值蓋掉。
  同一組欄位裡的 `SwReviewer`／`DaReviewer` 是業務流程紀錄（誰做了 115 上的檢視），
  仍然由人挑選，不自動寫入。
...
- **表格排序在前端**：資料量皆在 50 筆內；業務端 0904 確認不需要分頁，因此維持現狀。
  ...
  表頭不參與。六張主要清單與共用的人員表已經有；其餘維護清單還沒掛，要的話加一個 class 即可。
- **權限設定的使用者篩選在前端**：那個畫面點一位使用者就整頁重載（網址帶 `userId`），
  伺服器端篩選要把條件一路帶進每個連結與存檔後的導回；而 135 位使用者本來就全部
  印在畫面上了，直接藏起來最單純。條件記在 `sessionStorage`，否則挑完人重載就沒了。
