...
| 來源 | 放什麼 | 誰維護 |
|------|-------|-------|
| `FieldOptions`（寫死） | 本質上不會變的：是／否、等級 1~4、拋入／拋出 | 工程師改程式 |
| `Departments`／`Employees`／`OpsStaffs` | 組織資料：單位、人員 | 管理者，各有專屬畫面 |
| `FieldOptionItems` ＋ `OptionCatalog` | 值固定但業務端要能自己增刪的欄位 | 管理者，六個欄位**共用一個畫面** |

第三種的六個欄位：

| 欄位 | 型式 | 側邊欄位置 |
|------|------|-----------|
| SW-權責單位 | 單選 | 維護資料 → 資訊資產清單-軟體(SW) |
| SW-位置 | 多選 | 同上 |
| SW-自行/委外開發 | 多選 | 同上 |
| SW-使用語言種類 | 多選 | 同上 |
| SW-軌跡留存方式 | 單選 | 同上 |
| DA-有無機敏資料 | 多選 | 維護資料 → 資訊資產清單-資料(DA) |
...
0903 已把 SW-位置／SW-自行委外開發／SW-使用語言種類／DA-有無機敏資料 四欄的
分隔符統一成 `/`（共 35 筆），並合併了兩組同義寫法：`沒有`→`無`、`JAVA`→`Java`。
...
- **`SW-位置` 有 3 筆的值不在選項中**：業務端指定只有信義IDC與板橋IDC，
  但既有資料另有 `用戶端元件`(1)、`分公司`(1)、`N/A`(1)。值不會被清掉（畫面標示
  「不在選項中」並保留），要嘛改資料、要嘛在維護畫面把它們加成正式選項。
- **`SW-軌跡留存方式` 與 `SW-使用語言種類` 的選項是照現有資料產生的**，
  其中 `資料庫 - etl 匯入變動紀錄`、`LogFile` 看起來應該併進 `資料庫`／`檔案`，
  `Genero/Informix-4GL` 被視為兩個語言，請業務端確認。
- **`SW-自行/委外維護` 沒有一起處理**：它和 `SW-自行/委外開發` 有一樣的 `;#` 問題，
  但這次的指示只提到開發那一欄。
