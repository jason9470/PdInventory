...
**發佈輸出不包含 `App_Data`**（`FolderProfile.pubxml` 的 `ExcludeApp_Data`）。
資料庫本體是甲方的正式資料，只能由 EF migration 在啟動時就地升級，不能靠複製 `.db` 檔。

> ⚠ 0917 發生過一次：當時發佈設定是 `ExcludeApp_Data=false`，上線環境的 `pdinventory.db`
> 被 repo 裡的開發用資料庫整個蓋掉，甲方 9/15～9/17 早上的異動（6 筆資產的維護人員、
> 代理人、檢視人員、程式設計人員、關連系統）跟著消失，登入紀錄也倒退。
> 補救在 `RestoreOverwrittenAssetEdits` migration，只補「被蓋掉而且之後沒人再動過」的欄位。
>
> repo 裡仍然保留一份 `App_Data/pdinventory.db` 當開發資料，內容定期與甲方現況同步；
> 它只是開發機用的，永遠不該出現在發佈輸出或上線環境。
...
新增與編輯畫面上，「SW-權責單位」是**組別**，人員欄位只列那一組的人：

| 欄位 | 畫面元件 | 來源 |
|------|---------|------|
| SW-權責單位 | 一般使用者只列**自己的組**，管理者列全部 | `LookupOptions.OwnerUnits` |
| SW-應用系統主管 | 單選（只列備註是組長／科長的人） | `_PersonSelect` + `EmployeeManagersWithTeam` |
| SW-115上檢視人員、DA-115檢視人員 | 單選 | `_PersonSelect` + `EmployeesWithTeam` |
| SW-應用系統維護人員、維護代理人、程式設計人員 | 多選勾選清單 | `_MultiCheckList` 的 `Teams` 參數 |

- 篩選在前端做（`site.js` 的「人員欄位依組別連動」）：選項一次全部印出來，掛上 `data-team`，
  換組別時只是顯示／隱藏。人員只有 133 位，換一次組別要重新跟伺服器要一輪反而慢。
- **已經選好的人一定留著**，即使不是那一組，並標示「（不在這個組）」——藏起來的話，
  使用者只是開了畫面按存檔，原本的值就沒了（`SelectListItem` 不能帶自訂屬性，
  所以人員單選改用 `_PersonSelect` 自己寫 `<option>`）。
- 權責單位空白、或選到不屬於任何組的值（「資訊系統開發一部」）時不篩，列全部的人。
- 管理者不受限的理由同 8.1：部室主管與副主管本來就不屬於任何一組，照組別篩會一組都選不到。
  這是操作防呆，不是權限控制——能不能改這筆資產仍然看科別（見 8.2）。
...
- **清除**：[清除]的連結帶 `clear`，搜尋字與組別都清掉。
- **只清搜尋字**：[取消]／[回列表]的連結帶 `back`（0918 起改的）——搜尋字清掉（否則回到清單只剩剛剛
  那一筆），**組別留著**：那是「我正在看哪一組」，看完一筆回到清單不該跳回全部組別。
- **使用**：直接進入清單頁（網址沒有 `q`）時帶出記憶值並套用篩選。

> `clear` 與 `q` 都以「網址有沒有這個參數」判斷，不用 action 參數：`bool` 參數只接受 `true`/`false`，`clear=1` 會綁不進來而靜默失效。

**組別篩選也一起記（0918）。** 六張清單的搜尋列都有一個「全部組別」下拉（`team`），
與搜尋欄在同一個表單裡：同一顆[搜尋]送出、同一顆[清除]清掉，記憶分開存但規則相同
（`SearchMemory.ResolveTeam`）。組別就是資訊資產的 `SW-權責單位`：SW 清單直接比那一欄，
其餘五張以 `SystemCode` 對回資訊資產（`Helpers/TeamFilter.cs`）——**沒有 SystemCode
或對不到資產的資料選了組別就不會出現**，它們不屬於任何一組。

[匯出 Excel]鈕移到搜尋列最右邊（`ms-auto`），連結會帶上目前的 `q` 與 `team`，
匯出的內容與畫面所見一致；清單與匯出共用同一支篩選方法，兩邊各寫一套遲早會對不上。
...
|------|---------------|---------------|------------------------|
| 檢視六張清單 | 全部 | 全部 | 全部 |
| **修改** | 全部 | **自己科別負責的系統**（組長＝全組），見 8.2 | 同左 |
| **新增** | ✔ | ✘ | ✘ |
| **刪除** | 全部 | **同修改：能改就能刪**（0918），見 8.6 | 同左 |
| 維護資料、權限設定 | 檢視＋修改 | **只能檢視**（0918） | ✘ |
| 維護匯出 | ✔ | ✘ | ✘ |

**0917 起角色只決定「能做哪些動作」，「能改哪些系統」由科別決定。** 原本主管不受資產限制、
全部系統都能改，改版後主管在六張清單上與一般使用者相同；0918 起主管多了**檢視**維護資料與
權限設定（含組織圖、負責系統圖、「使用中」明細），但畫面上沒有新增、編輯、刪除，直接打網址或
送出表單也會被擋。做法是這些控制器類別層級掛 `Policies.ViewMaintenance`（管理者＋主管），
每個會修改資料的動作再另外掛 `Policies.Admin`——兩個 `[Authorize]` 要同時通過。
主管的角色在人員表單上設定；角色記在登入 Cookie 裡，**改完要請對方重新登入才生效**。

保留「主管」這個角色也是因為業務端表示新增權限日後可能開放給主管——屆時改 `Program.cs` 的 `Policies.CreateAssets` 與
...
| `Policies.ViewMaintenance` | 管理者、主管 | 維護資料控制器、`Employees`、`Sections`、`Usage`、`Permissions`（轉址）的類別層級（檢視） |
| `Policies.Admin` | 管理者 | `ExportSettings`；上面那些控制器裡所有會修改資料的動作 |
...
**組織圖**（0918，人員畫面的[組織圖]鈕，`EmployeesController.OrgChart`）：部室 → 組 → 科 → 人，
全部由科別表與人員表即時畫出來，不另外維護一份。部室層級的第一個（部室主管）畫在最上面，
其餘（副主管）從主幹旁接出；組橫排，組底下的科**縱向**往下排——14 個科全部橫排會寬到要左右捲動。
組長掛在組上、科長與組員掛在科上，備註是「組長／科長」的人排在最前面。連接線全部用 CSS 畫，
可以直接列印（列印時自動隱藏側邊欄與按鈕）。

科別畫面有同一套畫法的**負責系統圖**（`SectionsController.Chart`）：科底下列負責的系統，
組的名牌是全組合計（組長的範圍），範圍一律走 `SectionScope`；沒有任何科負責的系統列在下方提醒。
兩張圖共用 `Views/Shared/_OrgTree.cshtml` 與 `OrgChartViewModel`，只有「每個方塊底下列什麼」不同。
