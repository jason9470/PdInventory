...
每次啟動都會補上缺的帳號（角色一律給最低的資產負責人，**已存在的不動**——
角色與名下資產都是設定過的）。沒有員工編號的人略過：兩張表以員編相認，
...
- **三張清單的刪除各自獨立**：SW 清單刪的是 `InfoSystem`，DA 清單刪的是 `DataAsset`，
  盤點表清單刪的是 `SystemInventory`。刪掉 SW 不會連帶清掉它的 DA 與盤點表資料。
  目前採軟刪除的是 `InfoSystem`、`InventoryItem`、`TransferRecord`、`Employee`、`AppUser`；
  `DataAsset` 與 `SystemInventory` 仍是硬刪除。
