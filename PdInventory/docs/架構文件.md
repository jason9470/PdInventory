...
| `EmployeesController` | **共用** | 人員（員編＋姓名＋部門＋組別＋科別）**，與登入帳號對接** | `/Employees` |
...
| 來源 | 欄位數 | 放什麼 | 誰維護 |
|------|-------|-------|-------|
| `FieldOptions`（寫死） | 19 | 邏輯上封閉的：是／否、有／無、Y／N、型錄常數 | 工程師改程式 |
| `Departments`／`Employees`／`OpsStaffs` | — | 組織資料：單位、人員 | 管理者，各有專屬畫面 |
| `FieldOptionItems` ＋ `OptionCatalog` | 18 | 值固定但業務端要能自己增刪的欄位 | 管理者，18 個欄位**共用一個畫面** |

判斷標準是「誰有權改它」：**是／否沒有第三個答案**，開放編輯只會讓人加出「也許」；
反過來說，只要是業務詞彙（時間級距、傳輸方式、版控工具…），歷史上就一定會長出新值。

第三種的十八個欄位（側邊欄依欄位所屬表單分組）：
...
**現有資料的每一個值都在選項清單裡，十八欄零例外。**

### 還留在 `FieldOptions` 的 19 個

其中 15 個是邏輯封閉的（是／否 6、是／否/NA 2、有／無/NA 2、Y／N 1、
資料控制者／資料處理者 1、拋入／拋出 1、軟體／資料 2），開放編輯沒有好處。

另外 **4 個是程式碼有依賴的**，要開放前得先加保護：

| 欄位 | 依賴 | 改壞的後果 |
|------|------|-----------|
| SW-機密性／完整性／可用性 | `AppDbContext.RecalculateAssetValues` 把三個值 `int.TryParse` 後相加 | 有人加一個非數字的選項，資產價值會**安靜地停止計算**，不報錯 |
| SW-系統類別 | `site.js` 的 `CORE_CATEGORY = '核心系統'` | 「核心系統」被改名，資產價值 > 10 的阻擋視窗就失效 |

要開放這四個，得先在 `OptionCatalog` 加上「系統保留值」的標記，
讓維護畫面不准刪、不准改那幾個特定選項。

> 六個欄位共用一個控制器、靠 `?field=` 區分（作法比照「維護匯出」的 `?list=`），
...
**兩個固定值欄位**：`SW-保管單位` 恆為「資訊系統開發一部」、`SW-風險擁有者` 恆為「陳映玲」
（業務端指定）。畫面上是唯讀輸入框，但唯讀只是操作防呆，改個表單欄位就繞得過，
因此 `AppDbContext` 存檔時會再蓋一次。異動時改 `FieldOptions.FixedCustodianUnit`／
`FixedRiskOwner` 兩個常數並重新部署——這是寫死的代價，若之後會換人就該改成維護資料表。
...
| Mode | 實作 | 用途 |
|------|------|------|
| `Simulated` | `SimulatedEmployeeDirectory` | 登入頁自行輸入或點選身分、不驗證密碼。沒有 AD 的開發環境用這個模式，姓名可在登入頁自行填寫 |
...
| 權限設定 `AppUsers` | 能不能登入、什麼角色、名下有哪些資產 |
...
1. **使用者自己先登入**——人員表沒有他就自動建一筆：員編與姓名取自 LDAP，
   ...
   管理者可以用備註把待補的人篩出來）。帳號給最低的資產負責人、名下沒有資產。
2. **管理者先在人員表建檔**——存檔時一併建立帳號（同樣預設資產負責人），
   管理者接著到權限設定調角色；那個人之後登入就直接對應到已經設好的權限，
...
- 通過後在 `Employees` 與 `AppUsers` 各加一筆刪除註記（`IsDeleted`／`DeletedAt`／`DeletedBy`），
  **資料不會真的從資料庫消失**。全域查詢篩選讓他從人員清單、所有人員下拉與權限設定中消失。
- 那個人**不能再登入**，會看到「這個帳號已被停用，請洽系統管理者。」——
  密碼其實是對的，所以不必再用含糊的訊息遮掩，否則他只會反覆重試。
- 同一個人日後再從人員表新增回來時，原本那筆帳號會被復原（角色與名下資產都還在），
  不會撞到編號唯一索引——兩張表的唯一索引都加了 `IsDeleted = 0` 的篩選。
...
- **被停用的人若 AD 帳號還在，登入會被擋下**。規格沒有說這種情況要怎麼處理，
  ...
  要放行的話，請管理者從人員表把那個人新增回來（原本的角色與名下資產會一併復原）。
- **`SW-使用語言種類` 的 `Genero/Informix-4GL` 被切成兩個語言**（Genero 與 Informix-4GL）。
  若它其實是一個名字，在維護畫面新增合併後的選項、把那筆資料改過去即可。
- **`SW-應用系統主管` 有 16 筆填的不是組長**：業務端指定這個下拉「只列科別為組長的人」，
  但既有資料裡 林大鈞(7)、施凱家(6)、邱永嬌(2)、張永顯(1) 都不是組長（另有 1 筆 `N/A`）。
  值不會被清掉——下拉會多一個「（目前值，不在選項中）」的選項並選中它——但這 16 筆
  要嘛改人、要嘛把那幾位的科別改成組長，請業務端確認。
- **維護資料表裡有一批「代號待補」的資料**：既有資料中出現、但不在甲方 0903 清單裡的
  單位（16 筆）、維運人員（3 筆）與人員（8 筆，含 奇唯／奇狐／嘉實／全景／敦陽(廠商)
